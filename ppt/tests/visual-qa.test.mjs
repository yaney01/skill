...
test('visual QA recognizes evidence semantics declared on the slide root', () => {
  ...
  const report = path.join(directory, 'report.json');
  const html = `<!doctype html><html><style>.slide{width:1920px;height:1080px;position:absolute;visibility:hidden}.slide.active,.slide.visible{visibility:visible}.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:40px}.card{min-height:240px;border:2px solid #111}</style><body><main><section class="slide active visible" aria-hidden="false" data-slide-id="slide-01" data-layout="three-up" data-visual-required="true" data-visual-type="html-visualization" data-visual-role="explanation"><h2>三个问题</h2><div class="grid"><div class="card">重复生产</div><div class="card">质量波动</div><div class="card">反馈失真</div></div></section><section class="slide" aria-hidden="true" data-slide-id="slide-02" data-layout="four-grid" data-visual-required="true" data-visual-type="html-visualization" data-visual-role="explanation"><h2>四项能力</h2><div class="grid"><div class="card">治理</div><div class="card">生产</div><div class="card">衡量</div><div class="card">学习</div></div></section></main></body></html>`;
  const manifest = {
    manifestVersion: 2,
