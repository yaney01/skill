...
Bundles local stylesheets, scripts, images, fonts, media, CSS url() assets, and a
sibling deck.json manifest into one HTML file. Remote URLs are preserved and
reported because this tool does not download network resources.`;
...
const defaultOutput = path.join(inputDirectory, `${path.basename(input, extension)}.bundled${extension || '.html'}`);
...
const mimeTypes = new Map([
  ...
  ['.mp4', 'video/mp4'], ['.webm', 'video/webm'], ['.json', 'application/json'], ['.pdf', 'application/pdf']
]);

function isRemote(reference) { return /^(?:https?:)?\/\//i.test(reference); }
function isEmbeddedOrFragment(reference) { return /^(?:data:|blob:|#|mailto:|tel:|javascript:)/i.test(reference); }
...
function toDataUri(filePath) {
  if (!fs.existsSync(filePath) || !fs.statSync(filePath).isFile()) throw new Error(`Referenced local asset does not exist: ${filePath}`);
...
function escapeStyleContent(value) { return value.replace(/<\/style/gi, '<\\/style'); }
function escapeScriptContent(value) { return value.replace(/<\/script/gi, '<\\/script'); }

let html = fs.readFileSync(input, 'utf8');
const manifestPath = path.join(inputDirectory, 'deck.json');
if (fs.existsSync(manifestPath) && fs.statSync(manifestPath).isFile()) {
  const manifest = escapeScriptContent(fs.readFileSync(manifestPath, 'utf8'));
  const block = `<script type="application/json" id="deckManifest">\n${manifest}\n</script>`;
  if (/<script\b[^>]*\bid=["']deckManifest["'][^>]*>[\s\S]*?<\/script>/i.test(html)) {
    html = html.replace(/<script\b[^>]*\bid=["']deckManifest["'][^>]*>[\s\S]*?<\/script>/i, block);
  } else {
...
html = await replaceAsync(html, /<script\b([^>]*\bsrc\s*=\s*(["'])(.*?)\2[^>]*)>\s*<\/script>/gi, async (full, attributes, quote, source) => {
  ...
  const cleanedAttributes = attributes.replace(/\s*src\s*=\s*(["']).*?\1/i, '').replace(/\s*defer\b/i, '').trim();
  stats.scripts += 1;
  return `<script${cleanedAttributes ? ` ${cleanedAttributes}` : ''} data-bundled-from="${source.replaceAll('"', '&quot;')}">\n${escapeScriptContent(code)}\n</script>`;
});

html = await replaceAsync(html, /<style\b([^>]*)>([\s\S]*?)<\/style>/gi, async (full, attributes, css) => `<style${attributes}>${escapeStyleContent(await embedCssUrls(css, inputDirectory))}</style>`);
...
if (unresolved.length > 0) throw new Error(`Unresolved local references remain: ${[...new Set(unresolved)].join(', ')}`);

fs.mkdirSync(path.dirname(output), { recursive: true });
fs.writeFileSync(output, html, 'utf8');
console.log(`Bundled: ${output}`);
console.log(`Inlined ${stats.stylesheets} stylesheet(s), ${stats.scripts} script(s), ${stats.assets} local asset reference(s), and ${stats.manifests} deck manifest(s).`);
